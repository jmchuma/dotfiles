au BufRead,BufNewFile *.{swg,swig,i}	setf swig
au BufRead,BufNewFile *.tex		setf tex
au BufRead,BufNewFile *.g		setf antlr3
au BufRead,BufNewFile *.json		setf json
au BufRead,BufNewFile *.fountain	setf fountain
" de momento no esta pero las hare
au BufRead,BufNewFile *.{j,jasmin}	setf jasmin
" la de stringtemplate
au BufRead,BufNewFile *.stg		setf stg
" this is temporary
au BufRead,BufNewFile *.tjj		setf trajaja

au BufNewFile,BufRead */git/config setf gitconfig
